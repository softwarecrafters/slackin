...
var gotcha = form.elements['gotcha']
...
body.addEventListener('submit', function (ev){
  ...
  invite(channel ? channel.value : null, coc && coc.checked ? 1 : 0, email.value, gcaptcha_response.value, gotcha.value, function (err, msg){
    if (err) {
...
function invite (channel, coc, email, gcaptcha_response_value, gotcha, fn){
  request
