...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    ...
    dom('p',
      ...
      dom('a', { href: 'https://docs.google.com/document/d/1KPTc1SIFg5ftLpgYWk3go3LmlXUGpYZwrs10RrB_Y-I/edit', target: '_blank' }, 'FAQs'),
      ' and our ',
      dom('a', { href: coc, target: '_blank' }, 'Code of Conduct'),
      ' before joining. We reserve the right to exclude people from this community should they cause harm outlined by the CoC.'
    ),
    dom('p',
      dom('strong', 'We do not allow advertisements or unsolicited messages in this Slack.'),
    ),
    dom('p',
      dom('strong', 'Do not join if you intend to just post a job advertisment or looking to promote something.')
    ),
    dom('p.status',
      active
        ? [
    ...
    dom('form id=invite',
      ...
      dom('input.form-item type=email name=email placeholder=you@yourdomain.com '
        + (!iframe ? 'autofocus' : '')),
      dom('br'),
      dom('p', "Please type the following words in to the next textfield: ", dom("b", "I WILL NOT POST ADVERTISEMENTS"), " and take them to your heart."),
      dom("br"),
      dom("input.form-item type=text name=gotcha placeholder='I WILL NOT POST ADVERTISEMENTS'"),
      dom("br"),
